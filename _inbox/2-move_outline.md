...
5. if a source transcript file exists in the current directory, give it a new name and move it to the **same destination folder** as the outline; **do not** add any `files.json` entry for the transcript

6. append a new entry to **`files.json`** at the project root (for the outline only)

7. return only the final relative file path
...
- A meeting transcript may also be present in the current working directory, commonly named `transcript.txt` or similar (any reasonable transcript filename the workflow produced). If no transcript file is present, skip transcript handling entirely.

- **`files.json`** is at the **project root** (same directory as `Resources/`), not a separate `outlines.json`. Entries in `files.json` are **only** for the outline; never add a row for the transcript.
...
Move `outline.md` into **`Resources/`** (or the rare alternate destination you chose), rename it, and append a matching entry to **`files.json`**. When a companion transcript is present, preserve it in that same folder with a parallel filename, but index **only** the outline in `files.json`.
...
- the transcript file in the current directory, if one exists
...
Move `outline.md` into the chosen destination folder (almost always **`Resources/`**; use another existing top-level folder only when this repo already uses that pattern for the same content type) and rename it to the final filename.
...
STEP 7B — PRESERVE THE TRANSCRIPT (IF PRESENT)

--------------------------------

If a transcript file exists alongside `outline.md` in the current working directory:

1. **Destination:** move it into the **same folder** as the moved outline (usually `Resources/`, or the same rare alternate folder you used for the outline).

2. **New name:** base the name on the **same** `YYYY-MM-DD` and **same** `title-slug` as the final outline filename, with a clear transcript marker before the extension, for example:

   - `YYYY-MM-DD-title-slug-transcript.txt`

   Keep the original file extension when it matters (e.g. `.txt`, `.md`); the `-transcript` segment distinguishes it from the outline `.md` file.

3. **Collisions:** if that transcript path already exists, apply the same numeric suffix pattern as in Step 6 (`-2`, `-3`, …) to the **full basename** (or match whatever collision rule you used for the paired outline, so the pair stays consistent).

4. **`files.json`:** do **not** append an entry for the transcript. The transcript is stored next to the outline for reference only; the index documents the outline file alone.



If no transcript is present, omit this step.
...
After determining the destination folder (usually `Resources`), **category**, final filename, date, and display title, append a new object to the end of the JSON array in **`files.json`** at the project root. This object describes **the outline only**; the transcript is never listed here.
...
the final relative file path of the **moved outline** (not the transcript)
